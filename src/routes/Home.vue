  ...
  <div>
    ...
    >
      ...
      <div class="relative">
        ...
        >
          <div
            class="grid grid-cols-2 xs:grid-cols-3 gap-x-4 gap-y-1 xs:gap-x-2 sm:gap-x-2 items-center md:flex md:flex-row px-2 mt-1 sm:mt-3 w-max mx-auto"
            v-if="showFilters"
          >
            ...
            <div class="inline-flex items-center space-x-2 text-sm text-gray-600 dark:text-gray-400 py-1 pr-2">
              <Select
                v-model="limit"
                variant="sm"
                :options="[
    ...
    <div
      class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-10 gap-4 p-4 max-w-7xl mx-auto"
    >
...
import { useEmotes } from '../hooks/useGetEmotes'
...
function syncAppliedFilters() {
  ...
  if (isExactSearch.value !== false) appliedFilters.value++
  if (zeroWidth.value !== false) appliedFilters.value++
  if (animated.value !== false) appliedFilters.value++
  if (ignoreTags.value !== false) appliedFilters.value++
  if (limit.value !== 20) appliedFilters.value++
}

onMounted(syncAppliedFilters)

watch([isExactSearch, zeroWidth, animated, ignoreTags, limit], syncAppliedFilters)
